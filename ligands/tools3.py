...
def adjustMol1(xyz,atomtypes_par,coords_par,num_p1,factor):
	...
	for i in range(num_p):
		...
		vector1=np.array(np.subtract(coords_par[0],coords_par[1])/(np.linalg.norm(np.subtract(coords_par[0],coords_par[1]))),dtype=float)
		vector2=np.array(factor*xyz[i]/(np.linalg.norm(factor*xyz[i])),dtype=float)
...
def adjustMol2(xyz,atomtypes_par,coords_par,num_p2,num_p1):
	...
	for i in range(num_p):
		...
		vector1=np.array(np.cross(coords_par[3],coords_par[1])/(np.linalg.norm(np.cross(coords_par[3],coords_par[1]))),dtype=float)
		vector2=np.array(xyz[i+num_p1]/(np.linalg.norm(xyz[i+num_p1])),dtype=float)
		R=rotation_matrix_from_vectors(vector2, vector1)
		for p in range(len(coords_par)):
			coords_par2=np.dot(coords_par, R)
